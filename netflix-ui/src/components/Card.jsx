...
import { removeFromLikedMovies } from '../../src/store/index';
...
const addToList =async()=>{
    try{
     await axios.post(baseUrl + "/api/user/add",{email,data:movieData})
    }catch(err){
        console.log(err);
    }
}
    ...
    <Container
     ...
     { isHovered &&(
        <div className='hover'>
            ...
            <video src={video} autoPlay loop controls muted onClick={()=>navigate("/player")}></video>
            </div>
            <div className="info-container flex column">
                <h3 className='name' onClick={()=>navigate("/player")}>
                    {movieData.name}
                    </h3>
                    <div className="icons flex j-betweeen">
